...
const Completed = () => {
    ...
    const completedTickets = tickets?.filter(ticket => !ticket.is_canceled && ticket.past_order) || [];
        ...
        <SafeAreaView style={[style.area, { backgroundColor: theme.bg, paddingTop: 10 }]}>
            <ScrollView showsVerticalScrollIndicator={false}>
                {completedTickets.map((ticket, index) => (
                    <View key={index} style={{ padding: 5, marginTop: index > 0 ? 10 : 0 }}>
                        <View style={[style.shadow, { backgroundColor: theme.borderbg, borderRadius: 20, padding: 10 }]}>
                            <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                                <Image 
...
const Cancelled = () => {
    ...
    const cancelledTickets = tickets?.filter(ticket => ticket.is_canceled) || [];
